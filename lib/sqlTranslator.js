            ...
            if (filter2) {
                ...
                attrs.forEach(function (attr) {
                    ...
                    if (['$and', '$or', '$xor', '$not'].includes(attr)) {
                        ...
                        whereText += "(".concat(_this.translateFullTextSearch(filter2[attr], entity, alias), ")");
                    }
                            ...
                            if (typeof filter2[attr] === 'object') {
                                ...
                                if (predicate.startsWith('$')) {
                                    ...
                                    if (['$in', '$nin'].includes(predicate)) {
                                        var _a = _this.translateEvaluation(predicate, filter2[attr][predicate], entity2, alias, type, initialNumber, filterRefAlias), stmt = _a.stmt, cn2 = _a.currentNumber;
                                        whereText += " (`".concat(alias, "`.`").concat(attr, "` ").concat(stmt, ")");
                                        currentNumber = cn2;
                                    }
                                    else {
                                        whereText += " (`".concat(alias, "`.`").concat(attr, "` ").concat(_this.translatePredicate(predicate, filter2[attr][predicate], type), ")");
                                    }
                                }
                                else {
                                    // 对object数据的深层次查询，这里必须要假设该数据是json
                                    (0, assert_1.default)(['object', 'array'].includes(type2));
